...
class Person{
    ...
    protected: int* prefs;   
    private: int number;  // unique for person of a given sex.
    protected: int match; //  number of woman/man person is matched to
    ...
    *@param numberOfPerson Is an identifying integer.*/
    ...
    public: bool isFree() { return is_free;} 
...
class Man : public Person {
    ...
    *@param numberOfMan Is an identifying integer.*/
    public: Man(int prefsList[], int numberOfMan) 
      :Person(prefsList, numberOfMan) {
	next_woman = 0; }
...
/** A man is a Person. She is the one who chooses whether or not to accept a propsal. 
...
class Woman : public Person {
    ...
    * @param numberOfWoman the womans' identifying number*/
    public: Woman(int prefsList[], int rprefs[], int length, int numberOfWoman) 
      : Person(prefsList, numberOfWoman) {
	revPrefs = rprefs;
	for(int pref = 0; pref < length; ++pref) {
	    int womanNum = prefs[pref];
	    revPrefs[womanNum - 1] = pref + 1;}}
...
void fillWomanArray(Woman women[], std::istream& r, int numOfMarriages) {
    int womanNum;
    int prefs[numOfMarriages];
    int revPrefs[numOfMarriages];
    for(int i = 1; i <= numOfMarriages; ++i) {
	r >> womanNum;
	for(int j = 0; j < numOfMarriages; ++j)
	    r >> prefs[j];
	women[i - 1] = Woman(prefs, revPrefs, numOfMarriages, womanNum); }}
